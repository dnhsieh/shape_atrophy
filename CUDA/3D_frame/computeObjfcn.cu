...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, mxArray const *prhs[])
{
	...
	mxGPUArray const *elmVtxMat, *vtxElmMat, *nmlVtxMat, *tsvVtxMat, *femVtxMat, *femIfoMat;
	...
	nmlVtxMat                 =  mxGPUCreateFromMxArray(prhs[ 5]);
	...
	fcnObj.elm.d_nmlVtxMat = (int    *) mxGPUGetDataReadOnly(nmlVtxMat);
	...
	if ( error != cudaSuccess )
		mexErrMsgIdAndTxt("objfcn3Dframe:cudaMalloc", "Fail to allocate device memory.");
	...
	{
		mexErrMsgIdAndTxt("objfcn3Dframe:memAssign", 
	...
	mxGPUDestroyGPUArray(nmlVtxMat);
