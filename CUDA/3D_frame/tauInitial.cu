...
__global__ void tauInitialKernel(double *d_tauVec,
                                 double *d_lmkMat, double tauCenXVal, double tauCenYVal, double tauCenZVal,
                                 double tauHgtVal, double tauRadVal, int tauPowInt, int lmkNum)
{
	...
	if ( lmkIdx < lmkNum )
	{
		...
		double zDif   = lmkVec.z - tauCenZVal;
		double radSqu = (xDif * xDif + yDif * yDif + zDif * zDif) / tauRad2;
                ...
                double *d_lmkMat, double *h_tauVarVec, double *h_tauPrmVec, int lmkNum)
{
	...
	double tauCenZVal = h_tauVarVec[2];
	double  tauHgtVal = h_tauVarVec[3];
	double  tauRadVal = h_tauPrmVec[0];
	int     tauPowInt = h_tauPrmVec[1];

	int blkNum = (lmkNum - 1) / BLKDIM + 1;
	tauInitialKernel <<<blkNum, BLKDIM>>> (d_tauVec,
	                                       d_lmkMat, tauCenXVal, tauCenYVal, tauCenZVal, tauHgtVal,
                                 ...
                                 double tauHgtVal, double tauRadVal, int tauPowInt, int lmkNum)
{
	...
	if ( lmkIdx < lmkNum )
	{
		...
		double *d_dczTauVec = d_duTauMat + 2 * lmkNum;
		double *d_dhTauVec  = d_duTauMat + 3 * lmkNum;
		...
		d_dczTauVec[lmkIdx] = (radSqu < 1.0 ? -2.0 * DTauVal / tauRad2 * zDif : 0.0);
                ...
                double *d_lmkMat, double *h_tauVarVec, double *h_tauPrmVec, int lmkNum)
{
	...
	tauInitialKernel <<<blkNum, BLKDIM>>> (d_tauVec, d_duTauMat, d_lmkMat,
	                                       tauCenXVal, tauCenYVal, tauCenZVal, tauHgtVal,
	                                       tauRadVal, tauPowInt, lmkNum);
