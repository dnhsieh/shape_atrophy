...
void objfcn(double *h_objPtr, double *h_posVec, fcndata &fcnObj)
{
	...
	computeFrame(fcnObj.d_tanNowMat, fcnObj.d_tsvNowMat, fcnObj.prm.d_lmkIniMat, fcnObj);
	assembleFemMatrix(fcnObj.d_ppdNowMat, fcnObj.d_ggdNowMat,
	                  fcnObj.d_lmkNowEdgMat, fcnObj.d_tanNowMat, fcnObj.d_tsvNowMat, fcnObj);
	...
	for ( int timeIdx = 0; timeIdx < timeNum - 1; ++timeIdx )
	{
		...
		vertexToEdge(fcnObj.d_lmkNxtEdgMat, fcnObj.d_lmkNxtMat,
		             fcnObj.elm.d_elmVtxMat, lmkNum, elmNum);
		computeFrame(fcnObj.d_tanNxtMat, fcnObj.d_tsvNxtMat, fcnObj.d_lmkNxtMat, fcnObj);
		assembleFemMatrix(fcnObj.d_ppdNxtMat, fcnObj.d_ggdNxtMat,
		                  fcnObj.d_lmkNxtEdgMat, fcnObj.d_tanNxtMat, fcnObj.d_tsvNxtMat, fcnObj);
		...
		cudaMemcpy(fcnObj.d_tanNowMat, fcnObj.d_tanNxtMat,
	...
	double *d_lmkEndMat = fcnObj.d_lmkStk + (timeNum - 1) * lmkNum * DIMNUM;
	varifold(h_objPtr, d_lmkEndMat, fcnObj);
